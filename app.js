...
const queen = '<i class="fas fa-chess-queen" style="color:#008000"></i>';
...
class Queen {
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#808080")
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "#D3D3D3");
        }
    }
